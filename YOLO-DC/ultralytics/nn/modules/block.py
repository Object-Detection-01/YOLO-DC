...
# 本篇文章（https://ieeexplore.ieee.org/document/9792391）提出了GFL（了Generalized Focal Loss）。
# GFL具体又包括Quality Focal Loss（QFL）和Distribution Focal Loss（DFL），其中QFL用于优化分类和质量估计联合分支，DFL用于优化边框分支。
...
# 三个MaxPool 串行连接，kerner size都是5*5。效果等价于SPP，但是运算量从原来的5*5 + 9*9 + 13*13 = 275 减少到了3*5*5 = 75
...
class C2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in
                                 range(n)))  # a通过n个串联的Bottleneck后的到m,m的大小是（b,c,w,h）
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in
                               range(n))  # n个Bottleneck组成的ModuleList,可以把m看做是一个可迭代对象
