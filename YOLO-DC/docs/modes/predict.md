...
`model.predict` accepts multiple arguments that control the prediction operation. These arguments can be passed directly
to `model.predict`:
!!! example
...
Class reference documentation for `Results` module and its components can be
found [here](../reference/yolo/engine/results.md)
